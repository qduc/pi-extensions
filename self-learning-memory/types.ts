...
export interface DurableMemory { id: string; type: "preference" | "fact" | "constraint" | "decision"; scope: Scope; statement: string; rationale: string; confidence: number; createdAt: string; confirmedAt: string; sourceSessionIds: string[]; evidence: EvidenceRef[]; revisions: { at: string; statement: string; reason: string }[]; archived?: boolean; }
...
export interface MemoryState { version: 1; sessions: StoredSession[]; candidates: MemoryCandidate[]; outcomes: CandidateOutcome[]; memories: DurableMemory[]; }
...
export interface MemoryEngine { completeSession(session: CompletedSession): Promise<StoredSession>; retrieve(request: RetrievalRequest): Promise<RetrievalResult>; inspect(id: string): Promise<DurableMemory | undefined>; searchSessions(query: string, limit?: number): Promise<StoredSession[]>; propose(candidate: Omit<MemoryCandidate, "id" | "createdAt">): Promise<MemoryCandidate>; forget(memoryId: string, reason: string): Promise<MemoryCandidate>; runExtraction(sessionId?: string, signal?: AbortSignal): Promise<MemoryCandidate[]>; runConsolidation(limit?: number, signal?: AbortSignal): Promise<CandidateOutcome[]>; }
