...
test("Pi adapter registers the required lifecycle and model-facing interfaces", async () => {
	...
	for (const tool of ["memory_retrieve", "memory_inspect", "memory_search_sessions", "memory_propose", "memory_forget", "memory_purge", "memory_status", "memory_review", "memory_extract", "memory_consolidate"]) assert.match(await readFile(new URL("./tools.ts", import.meta.url), "utf8"), new RegExp(`name: \\"${tool}\\"`));
