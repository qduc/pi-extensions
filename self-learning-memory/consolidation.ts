...
import type { CandidateOutcome, ConsolidationDecision, DurableMemory, MemoryCandidate, MemoryState, ModelInvoker, ModelProfile } from "./types.ts";

function fallback(candidate: MemoryCandidate): ConsolidationDecision { return { candidateId: candidate.id, action: candidate.suggestedAction === "forget" ? "forget" : (candidate.explicit || candidate.confidence >= 0.8 ? "create" : "defer"), reason: "deterministic conservative policy" }; }
const ACTIONS = new Set(["create", "merge", "update", "reject", "defer", "forget"]);
const DURABLE_TYPES = new Set(["preference", "fact", "constraint", "decision"]);
...
export async function consolidate(state: MemoryState, candidates: MemoryCandidate[], invoker?: ModelInvoker, model?: ModelProfile, signal?: AbortSignal): Promise<CandidateOutcome[]> {
	let decisions = candidates.map(fallback);
	if (invoker && model && candidates.length) {
		const result = await invoker.invoke({ purpose: "consolidate", model, input: { candidates, memories: state.memories }, signal });
	...
	for (const decision of decisions) {
		...
		if (decision.action === "create") {
			if (!["preference", "fact", "constraint", "decision"].includes(candidate.type)) { outcomes.push({ candidateId: candidate.id, action: "deferred", reason: "non-durable candidate type", decidedAt: now }); continue; }
			...
			const target = state.memories.find((memory) => memory.id === memoryId && !memory.archived);
		...
		} else if (decision.action === "forget") { const target = state.memories.find((memory) => memory.id === memoryId && !memory.archived); if (!target) { outcomes.push({ candidateId: candidate.id, action: "deferred", reason: "target memory not found", decidedAt: now }); continue; } target.archived = true; target.revisions.push({ at: now, statement: target.statement, reason: decision.reason }); }
